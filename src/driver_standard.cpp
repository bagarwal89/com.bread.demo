...
#include "update.h"
#include "plot.h"
#include "setup_nscompressible.h"
#include <stdio.h>
...
void Driver_Standard::operator () () {
  ...
  switch (run.getInt("case")) {
   ...
   // case 2:  {setup = new Setup_Burgers          (&control,&time,&grid,&init,&boundary,&fluxes,&source,&update,&plot,&postprocess); break;}
    case 4:  {setup = new Setup_NSCompressible   (&control,&time,&grid,&init,&boundary,&fluxes,&source,&update,&plot,&postprocess); break;}
    default: {printf("\nno case defined!\n"); return; break;}
  }
  (*init) (grid);
  //
